...
func mint{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(to: felt) {
    ...
    // Get current id from counter
    let (current_id: Uint256) = counter.read();

    // Advance the id by Uint256(1,0)
    let (new_token_id, _) = uint256_add(current_id, Uint256(1,0));

    // Write to counter, og_owner and mint
